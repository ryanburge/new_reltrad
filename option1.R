...
#####> naffil xaffil####
gss1 <- gss1 %>% 
  mutate(naffil = recode(relig, "1=1;2=4;3=5;4=9;5:10=6;11=1;12=6;13=1;else=0")) %>% 
  mutate(xaffil = factor(naffil,levels = c(0,1,4,5,6,9), labels = c(NA,"prot","cath","jew","other","nonaf")))


#####> Black Protestants ####
gss1 <- gss1 %>% 
  mutate(xbp = recode(other,"7=1;14=1;15=1;21=1;37=1;38=1;56=1;78=1;79=1;85=1;86=1;87=1;88=1;98=1;103=1;104=1;128=1;133=1;else=0")) %>% 
  mutate(
    xbp = case_when(
...
#####> xtn inter ####
gss1 <- gss1 %>% 
  mutate(xtn = recode(relig, "11=1;else=0")) %>%
  ...
  mutate(inter = recode(relig,"13=1;else=0")) %>% 
  mutate(
    xev = case_when(
      inter == 1 ~ 1,TRUE ~ xev
    )
...
graph <- bind_rows(graph_new, graph_old)
...
graph %>% 
  ...
  theme_gg("Abel") +
  y_pct() + 
  add_text(x = 2005.75, y = .31, word = "New", sz = 4) +
  add_text(x = 2005.7, y = .26, word = "Old", sz = 4) +
  labs(x = "", y = "", title = "Adding All Non-Denoms to the Evangelical Category", caption = "Data: GSS 1972-2018") +
  ggsave("D://new_reltrad/images/option1.png", type = "cairo-png", width = 8)
